          ...
          <main className="flex-1 min-w-0 min-h-0 relative flex flex-col bg-transparent overflow-hidden"
                ...
                <>
                  {activeChat !== 'global' && (() => {
                      ...
                      <div className="bg-[#121B2A]/95 backdrop-blur-md border-b border-[#D4AF37]/30 px-6 py-4 flex items-center justify-between sticky top-0 z-20 shadow-lg">
                          <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-full bg-[#1A2639] border border-[#D4AF37]/50 flex items-center justify-center overflow-hidden shadow-sm relative cursor-pointer" onClick={() => setSelectedUserModal(targetUser as any)}>
                              ...
                              <div className="flex flex-col">
                                 <span 
                                    ...
                                    onClick={() => setSelectedUserModal(targetUser as any)}
                                 >
              ...
              <div className="flex-1 overflow-y-auto px-2 md:px-4 py-2 space-y-1.5 scrollbar-thin">
                  {messages.filter(m => m && m.sender).filter(m => {
                     ...
                     return (
                         <div key={m.id || idx} className="flex justify-start px-1 md:px-2 group">
                             {isLiz ? (
                                 <div className="flex gap-2 w-full max-w-[98%] mt-1">
                                     <div className="relative shrink-0 mt-1 cursor-pointer" onClick={() => senderInfo && setSelectedUserModal(senderInfo)}>
                                     ...
                                     <div className="flex flex-col border-l-[3px] border-[#D4AF37]/20 pl-2.5 ml-1 flex-1 min-w-[200px]">
                                         <div className="flex flex-col w-full relative">
                                             <span 
                                                className="font-bold text-[#D4AF37] text-[13px] relative z-20 mb-1 cursor-pointer hover:underline"
                                                onClick={() => setInputValue(prev => prev + `@${m.sender} `)}
                                             >
                                                ELIZABETH {m.isAi && '(IA Administradora Gemini ✨)'}
                                             </span>
                                             
                                             {m.replyTo && (
                                                <div className="bg-white/5 border-l-2 border-[#D4AF37]/50 px-2 py-1 mb-1 rounded text-xs text-gray-400 italic flex flex-col">
                                                    <span className="font-bold text-[#D4AF37]">{m.replyTo.sender}</span>
                                                    <span className="truncate">{m.replyTo.text}</span>
                                                </div>
                                             )}
                                             
                                             <div className="flex flex-wrap items-end justify-between gap-2">
                                                <span className="text-[#E8D9B0] text-[14px] leading-snug flex-1 cursor-pointer hover:bg-white/5 rounded px-1 transition-colors" onClick={() => setReplyingTo(m)}>{m.text}</span><button onClick={() => setReplyingTo(m)} className="opacity-0 group-hover:opacity-100 transition-opacity text-[#D4AF37] hover:text-white p-1" title="Responder"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 14 4 9 9 4"></polyline><path d="M20 20v-7a4 4 0 0 0-4-4H4"></path></svg></button>
                                                <span className="text-[#8B98B0] text-[11px] font-mono shrink-0 ml-auto pl-2">{timeStr}</span>
                                             </div>
                                         </div>
                                         {m.image && <div className="mt-1.5"><img referrerPolicy="no-referrer" src={m.image} className="rounded-xl border border-white/10 max-w-full shadow-md h-28 object-cover cursor-pointer hover:opacity-80" onClick={() => setReplyingTo(m)} alt="adjunto"/></div>}
                                 ...
                                 <div className="flex gap-2 w-full mt-1.5 group">
                                     ...
                                     <div className="bg-[#F2E3C6] rounded-[20px] rounded-tl-sm px-3.5 py-2 max-w-[85%] shadow-sm flex flex-col relative min-w-[150px]">
                                         <span 
                                            className="font-bold text-[#5A52A5] text-[13px] mb-1 cursor-pointer hover:underline"
                                            onClick={() => setInputValue(prev => prev + `@${m.sender} `)}
                                         >
                                            {m.sender}
                                         </span>
                                         ...
                                         {m.replyTo && (
                                            <div className="bg-black/5 border-l-2 border-[#5A52A5]/50 px-2 py-1 mb-1 rounded text-xs text-gray-700 italic flex flex-col">
                                                <span className="font-bold text-[#5A52A5]">{m.replyTo.sender}</span>
                                                <span className="truncate">{m.replyTo.text}</span>
                                            </div>
                                         )}

                                         <div className="flex flex-wrap items-end justify-between gap-2">
                                             <span className="text-[#1A2035] text-[14px] leading-snug flex-1 cursor-pointer hover:bg-black/5 rounded px-1 transition-colors" onClick={() => setReplyingTo(m)}>{m.text}</span><button onClick={() => setReplyingTo(m)} className="opacity-0 group-hover:opacity-100 transition-opacity text-[#5A52A5] hover:text-[#1A2035] p-1" title="Responder"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 14 4 9 9 4"></polyline><path d="M20 20v-7a4 4 0 0 0-4-4H4"></path></svg></button>
                                         ...
                                         {m.image && <div className="w-full mt-1.5"><img referrerPolicy="no-referrer" src={m.image} className="rounded-xl border border-black/10 max-w-full shadow-md h-28 object-cover cursor-pointer hover:opacity-80" onClick={() => setReplyingTo(m)} alt="adjunto"/></div>}
              ...
              <div className="px-2 pb-2 pt-1 shrink-0 bg-transparent relative z-10 max-w-5xl w-full mx-auto flex flex-col gap-2">
                  {replyingTo && (
                     <div className="bg-[#121B2A]/80 border border-[#D4AF37]/50 rounded-xl p-2 flex items-center justify-between shadow-lg mx-2">
                        <div className="flex flex-col">
                            <span className="text-[#D4AF37] text-xs font-bold flex items-center gap-1"><MessageCircle size={12}/> Respondiendo a {replyingTo.sender}</span>
                            <span className="text-gray-300 text-sm truncate max-w-[200px] sm:max-w-[400px]">{replyingTo.text || (replyingTo.image ? 'Imagen adjunta' : 'Audio')}</span>
                        </div>
                        <button onClick={() => setReplyingTo(null)} className="text-gray-400 hover:text-white p-1 bg-white/5 rounded-full"><X size={14}/></button>
                     </div>
                  )}
                          ...
                          {isRecording ? (
                              ...
                              <>
                                  <input 
                                     ...
                                     id="chat-input-field" placeholder="Escribe tu mensaje... @Elizabeth"
                                  />
