...
import {
  ...
  updateAiProfileInFirebase,
  saveMessageToFirebase,
} from "./server/firebaseLogic";
...
async function moderateMessage(
  ...
  try {
    const parts: any[] = [];
    parts.push({
      text: `Analiza este mensaje de chat. ¿Contiene insultos explícitos, groserías graves hacia otro usuario, violencia explícita, contenido sexual, o enlaces maliciosos? Si el usuario está bromeando de forma inofensiva o no hay insultos, debe pasar libremente.
...
Mensaje de texto: ${msg.text || "[Ninguno]"}`,
    ...
    if (
      ...
      if (matches && matches.length === 3) {
        ...
        parts.push({
          text: `Por favor escucha el audio adjunto, transcríbelo en el campo "transcription" del JSON y aplica la misma moderación.`,
      }
    }
...
async function startServer() {
  ...
  const top30Songs = [
    ...
    {
      title: "La Célula Que Explota - Caifanes",
          ...
          snapshot.docChanges().forEach((change) => {
            if (change.type === "modified" || change.type === "added") {
              ...
              if (data.username === "Elizabeth") {
                ...
                (activeUsers[data.username] as any).pais_idioma =
                  data.pais_idioma;
                changed = true;
              }
            }
  ...
  const emitActiveUsers = () => {
    const usersList = Object.values(activeUsers).map((u) => ({
      ...
      friends_list: (u as any).is_friends_public
        ? (u as any).friends_list
        : undefined,
  ...
  io.on("connection", (socket) => {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            if (owned.includes(decorationId))
              return callback({
                success: false,
                error: "Ya posees esta decoración",
            if (coins >= price) {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            if (decorationId && !owned.includes(decorationId))
              return callback({
                success: false,
                error: "No posees esta decoración",
            success = true;
          }
        } catch (e) {
        }
    ...
    socket.on("song_request", async (data) => {
      ...
      const msg = {
        id: Date.now().toString(),
        text:
          "He recibido tu solicitud para la canción '" +
          data.title +
          "'. ¿Es esta la canción que deseas enviar?",
        sender: "Elizabeth",
        isAi: true,
    ...
    socket.on("confirm_song_request", async (song) => {
      const msg = {
        id: Date.now().toString(),
        text:
          "¡Excelente! Estoy procesando tu solicitud para '" +
          song.title +
    ...
    socket.on(
      "dj_handle_request",
      (data: { id: string; action: "accept" | "reject" }) => {
        if (!currentUsername || currentUsername !== currentLiveDJ) return;
    ...
    socket.on("send_global", async (msg) => {
      ...
      if (
        ...
        const remaining = Math.ceil(
          (bannedUsers[currentUsername] - Date.now()) / 60000,
        );
        socket.emit("receive_global", {
          text: `🚫 Estás baneado por ${remaining} minutos más. No puedes enviar mensajes.`,
          sender: "Sistema",
          id: Date.now().toString(),
        return;
      }
      ...
      if (modResult.banned) {
        ...
        const banMsg = {
          text: `🚨 El usuario ${currentUsername} ha sido baneado por 15 minutos debido a: ${modResult.reason}.`,
          sender: "Elizabeth",
          id: Date.now().toString(),
          createdAt: Date.now(),
        };
      ...
      let isTrivia = false;
      if (msg.text && msg.text.trim().toLowerCase().startsWith("/trivia")) {
        ...
        try {
          ...
          if (fdb) {
            ...
            const snapshot: any = await Promise.race([
              getDocs(recentQ),
              new Promise((_, r) =>
                setTimeout(() => r(new Error("Firebase Timeout")), 3000),
              ),
            ]);
          ...
          let parts: any[] = [
            {
              text:
                ...
                contextMsgs
                  .map(
                    (m: any) =>
                      `[${new Date(m.createdAt?.seconds ? m.createdAt.seconds * 1000 : typeof m.createdAt === "number" ? m.createdAt : Date.now()).toLocaleTimeString()}] ${m.sender}: ${m.text}`,
                  )
                  .join("\n") +
                `\n\nResponde al último mensaje de ${currentUsername}.${(typeof isTrivia !== "undefined" ? isTrivia : (typeof isTriviaPrivate !== "undefined" ? isTriviaPrivate : false)) ? "\n\nEL USUARIO ACABA DE SOLICITAR UN JUEGO DE TRIVIA.\nINSTRUCCIÓN ESPECIAL PARA ELIZABETH: ¡Hazle una pregunta de trivia (cultura general, videojuegos, o ciencia)! Ofrece opciones A, B y C. Anímalo a jugar." : ""}`,
            },
          ];
          ...
          if (modResult.transcription) {
            parts.push({
              text: `[Nota: El usuario envió un audio que dice: "${modResult.transcription}"]`,
          }
...
Regla final: NO incluyas prefijos como 'Elizabeth:' al inicio de tu mensaje.`;
          ...
          const errorMsg = {
            text: "Uf, me quedé sin energía por un momento. Denme un respiro.",
            sender: "Elizabeth",
            id: Date.now().toString(),
            createdAt: Date.now(),
          };
    ...
    socket.on("send_private", async (msg, toUser, callback) => {
      ...
      if (
        ...
        return callback({
          success: false,
          error: "Estás baneado y no puedes enviar mensajes.",
      }
      ...
      let isTriviaPrivate = false;
      if (toUser === "Elizabeth" && msg.text && msg.text.trim().toLowerCase().startsWith("/trivia")) {
        isTriviaPrivate = true;
      }
      if (toUser === "Elizabeth") {
...
Regla final: NO incluyas prefijos como 'Elizabeth:' al inicio de tu mensaje.`;
          ...
          let parts: any[] = [
            {
              text:
                ...
                contextMsgs
                  ...
                  .join("\n") +
                `\n\nResponde al último mensaje de ${currentUsername}.${(typeof isTrivia !== "undefined" ? isTrivia : (typeof isTriviaPrivate !== "undefined" ? isTriviaPrivate : false)) ? "\n\nEL USUARIO ACABA DE SOLICITAR UN JUEGO DE TRIVIA.\nINSTRUCCIÓN ESPECIAL PARA ELIZABETH: ¡Hazle una pregunta de trivia (cultura general, videojuegos, o ciencia)! Ofrece opciones A, B y C. Anímalo a jugar." : ""} ${msg.text}`,
            },
          ];
          if (msg.image && msg.image.startsWith("data:image")) {
            const base64Data = msg.image.split(",")[1];
          ...
          console.error("Gemini Error:", e);
          const errorMsg = {
            text: "Uf, me quedé sin energía por un momento. Dame un respiro.",
            sender: "Elizabeth",
            id: Date.now().toString(),
            createdAt: Date.now(),
          };
    ...
    socket.on("accept_chess_invite", async (inviteData, callback) => {
      ...
      if (hostCoins < bet)
        return callback({
          success: false,
          error: "El anfitrión ya no tiene suficientes monedas.",
      if (!activeUsers[hostName])
        return callback({
          success: false,
          error: "El anfitrión ya no está en línea.",
    ...
    const handleChessGameOver = async (
      gameId: string,
    ...
    socket.on(
      "chess_bot_game_over",
      async (data: { gameId: string; result: string; winner: string }) => {
